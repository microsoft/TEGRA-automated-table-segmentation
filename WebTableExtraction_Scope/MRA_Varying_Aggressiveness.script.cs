...
public static class MyHelper
{
         ...
         string kbAttri2Values, string kbValue2Attris)
    {
        ...
        if (all_parameters.Contains(@"Jarrcard"))
            Parameter.d_occur_type = 2;

        Parameter.determine_num_cols_method = 3;
        Parameter.determine_num_cols_aggressiveness = Convert.ToInt32(splits_parametrs[8]);
        ...
        List<Table> aligned_table_wrapper = new List<Table>();

        ThreadStart starter = delegate { align_one_table(algorithm, lines, examples, numCols, aligned_table_wrapper); };

        //Give 30 minutes for a table to complete
        if (RunWithTimeout(starter, TimeSpan.FromMilliseconds(1000 * 60 * 30)))
        {
            Console.WriteLine("Worker thread finished.");
        }
        else
        {
            Table aligned_table_back = null;
            if (algorithm == "MRA")
            {
                //MRA is taking loo long, use Baseline instead
                Baseline baseline = new Baseline(lines, examples);
                if (Parameter.numColsGiven)
                    baseline.align(numCols);
                else
                    baseline.align();
                aligned_table_back = baseline.get_Extracted_Table();
            }
            ...
            else if (algorithm == "Judie")
            {
                ...
                if (Parameter.numColsGiven)
                    ...
                    judie.align();
                aligned_table_back = judie.get_Extracted_Table();
            }

            if (aligned_table_back != null)
                aligned_table_wrapper.Add(aligned_table_back);
            Console.WriteLine("Worker thread was aborted.");
        }


        Table aligned_table = aligned_table_wrapper[0];
    ...
    public static void align_one_table(string algorithm, List<Line> lines, Dictionary<Line, Record> examples, int numCols, List<Table> aligned_table_wrapper)
    {
        ...
        if (aligned_table != null)
            aligned_table_wrapper.Add(aligned_table);
